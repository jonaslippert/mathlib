...
| ty :=
  (do (_, lhs, _) ← relation_lhs_rhs ty, pure lhs)
    <|> pure ty
end

private meta def simp_nf (d : declaration) : tactic (option string) := do
...
cgr ← mk_congr_lemma_simp fn as.length,
